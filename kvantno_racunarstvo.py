...
class Falsifikator:
    ...
    def pokusaj_falsifikovanja(self, serijski_broj, kvantno_stanje):
        ...
        for _ in range(broj):
            stanje = random.choice(['0', '1', '+', '-'])
            pogodjeno_stanje.append('|' + stanje + '>')
        rezultat = None
        if(kvantno_stanje==pogodjeno_stanje):
            rezultat = "Validna novčanica"
        if rezultat:
            return   # Falsifikacija uspešna
        else:
            return False  # Falsifikacija neuspešna
